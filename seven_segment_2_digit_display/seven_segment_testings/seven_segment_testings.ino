...
void loop() {
  ...
  for (int n = 0; n < 500; n++) {
    ...
    digitalWrite(D1, LOW);  // Select the first digit (common anode: LOW to turn on)
    digitalWrite(D2, HIGH); // Deselect the second digit (common anode: HIGH to turn off)
  ...
  //TO SEE WHAT ACTUALLY IS GOING ON SET A DELAY HERE
  //delay(1000);
}
